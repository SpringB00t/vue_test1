        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSale of spuSaleAttrList" :key="spuSale.id">
                <dt class="title">{{ spuSale.saleAttrName }}</dt>
                <dd changepirce="0" :class="{ active: attr.newCheck == '1' }"
                  v-for="attr of spuSale.spuSaleAttrValueList" :key="attr.id"
                  @click="changeAttr(attr.baseSaleAttrId, attr.id)">{{ attr.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @input="test">
...
export default {
  ...
  mounted() {
    let aa = this.$store.dispatch("detail/getGoodsInfo", this.$route.params.skuid)
  },
  methods: {
    ...mapMutations("detail", ["CHANGEATTR"]),
    changeAttr(groupId, attrId) {
      this.CHANGEATTR({ groupId, attrId })
      // this.$store.commit("detail/CHANGEATTR", {groupId, attrId},groupId, attrId)
    },
    test() {
      if (this.skuNum.length == 1) { this.skuNum = this.skuNum.replace(/[^1-9]/g, '') } else { this.skuNum = this.skuNum.replace(/\D/g, '') }
